...
class MpyUartWindow(ed_basewin.EdBaseCtrlBox):
    """Control window for showing and running scripts"""
    def __init__(self, parent):
        super(MpyUartWindow, self).__init__(parent)
        ...
        if not handlers.InitCustomHandlers(ed_glob.CONFIG['CACHE_DIR']):
            util.Log(u"[MpyUart][warn] failed to load launch extensions")
        ...
        # Start the MpyUart uart serial COM-PORT interface
...
#             run_in_separate_thread=True)
        ...
        self.StartStopUart(run_during_startup=True)
...
#                                         wx.ALIGN_LEFT)
    ...
    def OnLexerChange(self, msg):
        ...
        """
        self._log("[MpyUart][info] Lexer changed handler - context %d" %
    ...
    def OnPageChanged(self, msg):
        ...
        if isinstance(ctrl, ed_basestc.EditraBaseStc):
            ...
            self._log("[MpyUart][info] Non STC object in notebook")
...
# 
#         util.Log("[MpyUart][info] Found commands %s" % repr(cmds))
...
#                 ctrl.Disable()
    ...
    def StartStopProcess(self):
        ...
        if self._busy:
            util.Log("[MpyUart][info] Starting process")
            handler = handlers.GetHandlerById(self.State['lang'])
            ...
            util.Log("[MpyUart][info] Aborting process")
            self._worker.Abort()
            self._worker = None


    def StartStopUart(self, run_during_startup=False):
        ...
        if self._busy:
            ...
            handler = handlers.GetHandlerById(self.State['lang'])
#            cmd = self.FindWindowById(ID_EXECUTABLE).GetStringSelection()
#            cmd = 'C:\projects\msp_editra\msppy_dev\msppy\prog.py'
            cmd = r'%s -u' % (self.python_exe)
            file = r'%s\mpy_uart\mpyuart\run_uart_comport.py' % (self.mpy_dir)
            self.State['lcmd'] = cmd
#            cmd = handler.GetCommand(cmd)
...
#         except TypeError:
#             util.Log("[MpyUart][err] UpdateCurrent Files: " + str(items))
    ...
    def DoProcessError(self, code, excdata=None):
        ...
        if excdata is not None:
            try:
                ...
                util.Log(u"[MpyUart][err] %s" % excdata)
            except UnicodeDecodeError:
                util.Log(u"[MpyUart][err] error decoding log message string")
