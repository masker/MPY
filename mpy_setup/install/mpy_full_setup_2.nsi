...
;  File /r "C:\MPY\*.*"
  ...
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\Scripts\editra.bat" "" "$INSTDIR\${MUI_ICON}"
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\Scripts\editra.bat" "" "$INSTDIR\${MUI_ICON}"
SectionEnd
...
Section "Context Menus" SEC02
  ...
  WriteRegStr HKCR "*\shell\OpenWithEditra\command" "" '"$INSTDIR\Scripts\editra.bat"  "%1"'
...
Section "Add Quick Launch Icon" SEC03
  SectionIn 1
  CreateShortCut "$QUICKLAUNCH\${PRODUCT_NAME}.lnk" "$INSTDIR\Scripts\editra.bat" "" "$INSTDIR\${MUI_ICON}"
SectionEnd
...
Section -AdditionalIcons
  ...
  SetShellVarContext all
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Website.lnk" "$INSTDIR\${PRODUCT_NAME}.url" "" "$INSTDIR\${MUI_UNICON}"
...
Function LaunchMpyEditor
  Exec '"C:\Python27\Scripts\editra.bat" "$INSTDIR\mpy_examples\exp6.mpy" '
FunctionEnd
...
Section "un.Program Data" UNSEC01
  ...
  ; Remove all Files
  RmDir /r "$INSTDIR\"
  RmDir /r "C:\Python27\Lib\site-packages"
